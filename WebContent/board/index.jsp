...
<div class="mt-3 mb-5">
	<div class="col-md-12 col-lg-12 col-xl-12 p-0" id="content"
		...
		<table class="table mb-0 mt-10 table-dark" border="0" cellspacing="0"
			cellpadding="0">
			<tr style ="background-color : #7075cc">
				<td class="display-3" align="center" style="font-size: 20px;">
					<div class="h3 row justify-content-center">
						...
						<select class="form-control border-0" id="monthSelect"
							style="font-size: 25px; color: white; width: 110px; background-color : #7075cc">
							<%
		...
		for (var i = 0; i < data.length; i++) {
			...
			if (dataMonth == month) {
				...
				if (data[i].inout_type.includes('입금')) {
					...
					// 일자에 출력해준다.
					if ($('#date' + dataDate).children('#deposit').length > 0) { // 입금 표시가 있다면 내용만 바꿔라
						$('#date' + dataDate).children('#deposit').empty();
						$('#date' + dataDate).children('#deposit').html( '+' + myDepositMap.get(dataDate));
					} else if ($('#date' + dataDate).children('#withdrawal').length > 0) { // 출금 표시가 있다면 입금을 출금표시 위에 올려야한다.
						$('#date' + dataDate).children('#withdrawal').before('<p class="deposit m-0" id="deposit" style="color:blue; font-size:' + textSize + ';">+' + myDepositMap.get(dataDate) + '</p>');
					} else {
